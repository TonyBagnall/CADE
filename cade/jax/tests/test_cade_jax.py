...
from cade.jax import cade_alignment_matrix, cade_grad_x, cade_loss
...
def test_cade_loss_equivalence(gamma, c, C, T, U):
    ...
    s_jax = float(cade_loss(x, y, c=c, gamma=gamma, reduction="none")[0])
...
@pytest.mark.parametrize("gamma", GAMMAS)
@pytest.mark.parametrize("c", CS)
def test_cade_alignment_matrix_equivalence(gamma, c):
    ...
    E_jax, s_jax = cade_alignment_matrix(x, y, c=c, gamma=gamma)
...
@pytest.mark.parametrize("gamma", GAMMAS)
@pytest.mark.parametrize("c", CS)
def test_cade_grad_x_equivalence(gamma, c):
    ...
    dx_jax, s_jax = cade_grad_x(x, y, c=c, gamma=gamma)
...
def test_cade_autograd_smoke():
    ...
    def loss_fn(x_in):
        return cade_loss(x_in, y, c=1.0, gamma=0.1, reduction="mean")
